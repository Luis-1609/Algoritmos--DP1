...
class Aeropuerto {
    ...
    int plazoMaximoHoras(Aeropuerto destino) {
        int diff = Math.abs(this.gmtOffset - destino.gmtOffset);
        return diff <= 4 ? 24 : 48;
    }
...
class Vuelo {
    ...
    int duracionMin() { return llegadaMin - salidaMin; }
...
class Pedido {
    ...
    int plazoMin(Map<String,Aeropuerto> aeropuertos) {
        ...
        if (ap == null || ad == null) return ingresoMin + 2880;
...
class RutaPedido {
    ...
    static RutaPedido directa(Vuelo v)              { return new RutaPedido(List.of(v)); }
    static RutaPedido conEscala(Vuelo v1, Vuelo v2) { return new RutaPedido(List.of(v1,v2)); }

    Vuelo primerVuelo() { return vuelos.get(0); }
    int   llegadaMin()  { return vuelos.get(vuelos.size()-1).llegadaMin; }
    ...
    boolean esConsistente() {
        for (int i = 0; i < vuelos.size()-1; i++) {
            ...
            if (!a.destino.equals(b.origen))    return false;
            if (b.salidaMin < a.llegadaMin)     return false;
        }
        return true;
    }

    RutaPedido copia() { return new RutaPedido(new ArrayList<>(vuelos)); }
